...
public class ImageViewerOverlay extends FrameLayout implements View.OnTouchListener {
    ...
    private void inflate(Context context) {
        ...
        findViewById(R.id.imageViewerOverlayAcibShrink).setOnClickListener(v -> zoomOutByButton(0.3f));
        findViewById(R.id.imageViewerOverlayAcibTurnLeft).setOnClickListener(v -> rotateByMatrix(-90));
        findViewById(R.id.imageViewerOverlayAcibTurnRight).setOnClickListener(v -> rotateByMatrix(90));
        findViewById(R.id.imageViewerOverlayAcibEnlarge).setOnClickListener(v -> zoomInByButton(0.3f));
    ...
    private boolean handleImageTouch(View v, MotionEvent event) {
        ...
        // 当旋转动画或矩阵过渡动画运行时，强制劫持触摸，保护核心变换状态机不发生死锁。
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (touchMode == TOUCH_EXIT_DRAG) {
                    // 下拉退出过程中的矩阵随动缩放与背景 Alpha 联动透明度渐变
                    ...
                    if (newDist > 5f) {
                        ...
                        // 动态解算手势最低下限，严格与按钮下限看齐，保证横竖屏自由缩小到相同尺寸。
                        float dynamicMinScale = Math.min(minScale, baseScale * 0.4f);
                        ...
                        // 引入平滑比例对冲，消灭图片极小时多点触控放大产生的瞬间大跳跃卡顿。
                        matrix.set(savedMatrix);
    ...
    private float calcOverscroll(RectF imageRect, boolean horizontal) {
        // 手势多点双指缩放期间 (TOUCH_PINCH) 或平滑缩放动画期间 (isAnimating)
        // 绝对不允许强制限制物理越界行为
        // 从而彻底保证在有大片留白的情况下能顺畅缩放到极小
        // 并无痕顺滑地拉大
    ...
    void onImageLoaded(Bitmap bitmap) {
        ...
        if ((getWidth() > 0) && (getHeight() > 0)) {
            ...
            // 如果首次渲染时 View 的物理宽高尚未测绘完毕
            // 通过 OnLayoutChangeListener 进行防抖兜底渲染
