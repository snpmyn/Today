...
public final class SpruceFlingAnimation extends SpruceDynamics<SpruceFlingAnimation> {
     ...
     */
    public SpruceFlingAnimation setFriction(@FloatRange(from = 0.0, fromInclusive = false) float friction) {
        if (friction <= 0) {
     ...
     * set the start velocity defaults to 0. In that
    ...
    boolean isAtEquilibrium(float value, float velocity) {
        return value >= mMaxValue || value <= mMinValue || mFlingForce.isAtEquilibrium(value, velocity);
    }
    ...
    static final class DragForce implements Force {
        ...
        private static final float VELOCITY_THRESHOLD_MULTIPLIER = 1000.0F / 16.0F;
        ...
        MassState updateValueAndVelocity(float value, float velocity, long deltaT) {
            mMassState.mVelocity = (float) (velocity * Math.exp((deltaT / 1000.0F) * mFriction));
            mMassState.mValue = value + (mMassState.mVelocity - velocity) / mFriction;
