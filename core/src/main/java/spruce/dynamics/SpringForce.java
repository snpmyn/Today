...
public final class SpringForce implements Force {
     ...
     */
    public void setStiffness(@FloatRange(from = 0.0, fromInclusive = false) float stiffness) {
        if (stiffness <= 0) {
     ...
     */
    public void setDampingRatio(@FloatRange(from = 0.0) float dampingRatio) {
        if (dampingRatio < 0) {
    ...
    public boolean isAtEquilibrium(float value, float velocity) {
        return (Math.abs(velocity) < mVelocityThreshold) && (Math.abs(value - getFinalPosition()) < mValueThreshold);
    }
    ...
    private void init() {
        ...
        if (mDampingRatio > 1) {
            // Over damping
            mGammaPlus = -mDampingRatio * mNaturalFreq + mNaturalFreq * Math.sqrt(mDampingRatio * mDampingRatio - 1);
            mGammaMinus = -mDampingRatio * mNaturalFreq - mNaturalFreq * Math.sqrt(mDampingRatio * mDampingRatio - 1);
     ...
     */
    SpruceDynamics.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        init();
        // unit: seconds
        ...
        if (mDampingRatio > 1) {
            // Over damped
            double coeffA = lastDisplacement - (mGammaMinus * lastDisplacement - lastVelocity) / (mGammaMinus - mGammaPlus);
            double coeffB = (mGammaMinus * lastDisplacement - lastVelocity) / (mGammaMinus - mGammaPlus);
            displacement = coeffA * Math.pow(Math.E, mGammaMinus * deltaT) + coeffB * Math.pow(Math.E, mGammaPlus * deltaT);
            currentVelocity = coeffA * mGammaMinus * Math.pow(Math.E, mGammaMinus * deltaT) + coeffB * mGammaPlus * Math.pow(Math.E, mGammaPlus * deltaT);
        } else if (mDampingRatio == 1) {
            // Critically damped
            double coeffB = lastVelocity + mNaturalFreq * lastDisplacement;
            displacement = (lastDisplacement + coeffB * deltaT) * Math.pow(Math.E, -mNaturalFreq * deltaT);
            currentVelocity = (lastDisplacement + coeffB * deltaT) * Math.pow(Math.E, -mNaturalFreq * deltaT) * -mNaturalFreq + coeffB * Math.pow(Math.E, -mNaturalFreq * deltaT);
        } else {
            // Under damped
            double sinCoeff = (1 / mDampedFreq) * (mDampingRatio * mNaturalFreq * lastDisplacement + lastVelocity);
            displacement = Math.pow(Math.E, -mDampingRatio * mNaturalFreq * deltaT) * (lastDisplacement * Math.cos(mDampedFreq * deltaT) + sinCoeff * Math.sin(mDampedFreq * deltaT));
            currentVelocity = displacement * -mNaturalFreq * mDampingRatio + Math.pow(Math.E, -mDampingRatio * mNaturalFreq * deltaT) * (-mDampedFreq * lastDisplacement * Math.sin(mDampedFreq * deltaT) + mDampedFreq * sinCoeff * Math.cos(mDampedFreq * deltaT));
        }
