 ...
 * in the animation is unhampered, the animation will never reach equilibrium. Instead, it will
 * oscillate forever.
 *
...
public final class SpruceSpringAnimation extends SpruceDynamics<SpruceSpringAnimation> {
     ...
     * @param <K>           the class on which the Property is declared
     */
    public <K> SpruceSpringAnimation(K object, FloatPropertyCompat<K> property, float finalPosition) {
        super(object, property);
        mSpring = new SpringForce(finalPosition);
    }
    ...
    /**
     * Skips to the end of the animation. If the spring is unhampered, an
     ...
     *
     * @throws IllegalStateException   if the spring is unhampered (i.e. damping ratio = 0)
    ...
    boolean updateValueAndVelocity(long deltaT) {
        ...
        if (mPendingPosition != UNSET) {
            ...
            massState = mSpring.updateValues(mValue, mVelocity, deltaT / 2);
            mSpring.setFinalPosition(mPendingPosition);
            mPendingPosition = UNSET;
            ...
            massState = mSpring.updateValues(mValue, mVelocity, deltaT);
        }
