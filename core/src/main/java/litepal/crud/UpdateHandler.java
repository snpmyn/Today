...
public class UpdateHandler extends DataHandler {
     ...
     */
    int onUpdate(@NonNull LitePalSupport baseObj, long id) throws SecurityException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
     ...
     * @return The number of rows affected.
     */
    public int onUpdate(Class<?> modelClass, long id, @NonNull ContentValues values) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (!values.isEmpty()) {
    ...
    int onUpdateAll(LitePalSupport baseObj, String... conditions) throws SecurityException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
     ...
     * @return The number of rows affected.
     */
    private int doUpdateAllAction(String tableName, @NonNull ContentValues values, String... conditions) {
        BaseUtility.checkConditionsCorrect(conditions);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (!values.isEmpty()) {
    ...
    private void putFieldsToDefaultValue(LitePalSupport baseObj, ContentValues values, long... ids) {
        ...
        try {
                    ...
                    if (isCollection(field.getType())) {
                        if (ids != null && ids.length > 0) {
                            ...
                            if (BaseUtility.isGenericTypeSupported(genericTypeName)) {
                                ...
                                StringBuilder whereClause = getStringBuilder(ids, genericValueIdColumnName);
            ...
            throw new LitePalSupportException(LitePalSupportException.noSuchFieldException(baseObj.getClassName(), fieldName), e);
        } catch (Exception e) {
    ...
    @NonNull
    private static StringBuilder getStringBuilder(@NonNull long[] ids, String genericValueIdColumnName) {
     ...
     * Unused currently.
     */
    private void updateSelfTableForeignKey(@NonNull LitePalSupport baseObj, ContentValues values) {
     ...
     * Unused currently.
     */
    private int updateAssociatedTableForeignKey(@NonNull LitePalSupport baseObj, long id) {
     ...
     *               value that will be translated to NULL.
     */
    private void convertContentValues(@NonNull ContentValues values) {
