...
public class DeleteHandler extends DataHandler {
    ...
    /**
     * Initialize {@link DeleteHandler#mDatabase} for operating database. Do not
     ...
     */
    int onDelete(@NonNull LitePalSupport baseObj) {
        if (baseObj.isSaved()) {
     ...
     * @return The number of rows affected. Including cascade delete rows.
     */
    public int onDelete(@NonNull Class<?> modelClass, long id) {
     ...
     */
    private void analyzeAssociations(@NonNull Class<?> modelClass) {
    ...
    private String buildConditionString(@NonNull String... conditions) {
     ...
     */
    private int deleteAssociatedForeignKeyRows(@NonNull LitePalSupport baseObj) {
        int rowsAffected = 0;
     ...
     */
    private int deleteAssociatedJoinTableRows(@NonNull LitePalSupport baseObj) {
        int rowsAffected = 0;
    ...
    private List<String> getForeignKeyTableToDelete() {
        if (foreignKeyTableToDelete == null) {
            foreignKeyTableToDelete = new ArrayList<>();
        }
     ...
     */
    private void deleteGenericData(Class<?> modelClass, @NonNull List<Field> supportedGenericFields, long... ids) {
        for (Field field : supportedGenericFields) {
            ...
            // prevent the where condition is too long
            int maxExpressionCount = 500;
