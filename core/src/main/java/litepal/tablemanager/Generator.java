...
public abstract class Generator extends LitePalBase {
     ...
     * method.
     *
     * @param sqlList SQLs that want to execute.
     * @param db      instance of SQLiteDatabase
     */
    protected void execute(List<String> sqlList, SQLiteDatabase db) {
        String throwSQL = "";
        try {
    ...
    private static void updateAssociations(SQLiteDatabase db) {
        AssociationUpdater associationUpdater = new Upgrader();
        associationUpdater.addOrUpdateAssociation(db, false);
    }
