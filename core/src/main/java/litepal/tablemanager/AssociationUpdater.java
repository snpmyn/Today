...
public abstract class AssociationUpdater extends Creator {
     ...
     */
    protected List<String> getForeignKeyColumns(@NonNull TableModel tableModel) {
    ...
    /**
     * The values in table_scheme should be synchronized with the model tables
    ...
    protected void clearCopyInTableSchema(List<String> tableNames) {
        if (tableNames != null && !tableNames.isEmpty()) {
            ...
            sqlList.add(deleteData.toString());
            execute(sqlList, mDb);
        }
    }
     ...
     */
    private String generateCreateNewTableSQL(@NonNull Collection<String> removeColumnNames, TableModel tableModel) {
     ...
     */
    protected String generateDataMigrationSQL(@NonNull TableModel tableModel) {
    ...
    private List<String> getRemoveColumnSQLs(Collection<String> removeColumnNames, String tableName) {
        ...
        List<String> sqlList = new ArrayList<>();
        sqlList.add(alterToTempTableSQL);
        sqlList.add(createNewTableSQL);
        sqlList.add(dataMigrationSQL);
        sqlList.add(dropTempTableSQL);
        sqlList.addAll(createIndexSQLs);
        return sqlList;
    }
     ...
     */
    private boolean isRelationCorrect(@NonNull AssociationsModel associationModel, String tableName1, String tableName2) {
