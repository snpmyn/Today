...
public class Upgrader extends AssociationUpdater {
                ...
                if (columnModelDB.getColumnName().equalsIgnoreCase(columnModel.getColumnName())) {
                    if (!columnModelDB.getColumnType().equalsIgnoreCase(columnModel.getColumnType())) {
                        if (columnModel.getColumnType().equalsIgnoreCase("blob") && TextUtils.isEmpty(columnModelDB.getColumnType())) {
                            LogUtils.i(this.getClass().getSimpleName(), "Case for binary array type upgrade. Do nothing under this condition.");
                        } else {
    ...
    private List<String> generateAddColumnSQLs(ColumnModel columnModel) {
        ...
        sqlList.add(generateAddColumnSQL(mTableModel.getTableName(), columnModel));
        if (columnModel.hasIndex()) {
            sqlList.add(generateCreateIndexSQL(mTableModel.getTableName(), columnModel));
        }
        return sqlList;
    }
     ...
     */
    @NonNull
    private List<String> getAddColumnSQLs(@NonNull List<ColumnModel> columnModelList) {
        List<String> sqlList = new ArrayList<>();
        for (ColumnModel columnModel : columnModelList) {
            sqlList.addAll(generateAddColumnSQLs(columnModel));
        }
        return sqlList;
    }
    ...
    private List<String> getChangeColumnsConstraintsSQL() {
        ...
        sqlList.addAll(addForeignKeySQLs);
