...
public class FluentQuery {
    ...
    public <T> T findFirst(Class<T> modelClass, boolean isEager) {
        synchronized (LitePalSupport.class) {
            ...
            if (!"0".equals(mLimit)) {
                // If mLimit not equals to 0, set mLimit to 1 to find the first record.
                mLimit = "1";
            }
            List<T> list = find(modelClass, isEager);
            mLimit = limitTemp;
            // Don't forget to change it back after finding operation.
            if (!list.isEmpty()) {
                if (list.size() != 1)
    ...
    public <T> T findLast(Class<T> modelClass, boolean isEager) {
        synchronized (LitePalSupport.class) {
            ...
            if (TextUtils.isEmpty(mOffset) && TextUtils.isEmpty(mLimit)) {
                // If mOffset or mLimit is specified, we can't use the strategy in this block to speed up finding.
     ...
     */
    public int count(@NonNull Class<?> modelClass) {
     ...
     */
    public double average(@NonNull Class<?> modelClass, String column) {
     ...
     */
    public <T> T max(@NonNull Class<?> modelClass, String columnName, Class<T> columnType) {
     ...
     */
    public <T> T min(@NonNull Class<?> modelClass, String columnName, Class<T> columnType) {
     ...
     */
    public <T> T sum(@NonNull Class<?> modelClass, String columnName, Class<T> columnType) {
