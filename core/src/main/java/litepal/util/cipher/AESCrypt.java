 ...
 * Created by scot tab on 04/10/2014.
 */
public final class AESCrypt {
    ...
    // toggle switch log option (please turn off in live!)
    ...
    public static String encrypt(final String password, String message) throws GeneralSecurityException {
        try {
            ...
            if (DEBUG_LOG_ENABLED) Timber.tag(TAG).e(e, "UnsupportedEncodingException ");
    ...
    private static void log(String what, byte[] bytes) {
        if (DEBUG_LOG_ENABLED)
            Timber.tag(TAG).d(what + "[" + bytes.length + "] [" + bytesToHex(bytes) + "]");
    }

    private static void log(String what, String value) {
        if (DEBUG_LOG_ENABLED) Timber.tag(TAG).d(what + "[" + value.length() + "] [" + value + "]");
    }

    /**
     * Converts byte array to hex decimal useful for logging and fault finding.
     *
     * @param bytes 字节
     * @return 字符串
     */
