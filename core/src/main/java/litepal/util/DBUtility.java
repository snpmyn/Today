...
public class DBUtility {
     ...
     *
     * @param foreignColumnName The foreign column name. Standard pattern is table name_id.
    ...
    public static String getM2MSelfRefColumnName(@NonNull Field field) {
    ...
    public static boolean isIntermediateTable(String tableName, SQLiteDatabase db) {
        if (!TextUtils.isEmpty(tableName)) {
            if (tableName.matches("[0-9a-zA-Z]+_[0-9a-zA-Z]+")) {
                try (Cursor cursor = db.query(Const.TableSchema.TABLE_NAME, null, null, null, null, null, null)) {
    ...
    public static List<String> findAllTableNames(SQLiteDatabase db) {
        List<String> tableNames = new ArrayList<>();
        try (Cursor cursor = db.rawQuery("select * from sqlite_master where type = ?", new String[]{"table"})) {
            if (cursor.moveToFirst()) {
                do {
    ...
    public static TableModel findPragmaTableInfo(String tableName, SQLiteDatabase db) {
        if (isTableExists(tableName, db)) {
            ...
            String checkingColumnSQL = "pragma table_info(" + tableName + ")";
            try (Cursor cursor = db.rawQuery(checkingColumnSQL, null)) {
                if (cursor.moveToFirst()) {
                    do {
    ...
    public static String convertWhereClauseToColumnName(String whereClause) {
        if (!TextUtils.isEmpty(whereClause)) {
            try {
                StringBuilder convertedWhereClause = new StringBuilder();
     ...
     */
    @NonNull
    private static String convertOrderByItem(@NonNull String orderByItem) {
        String column;
        String append;
