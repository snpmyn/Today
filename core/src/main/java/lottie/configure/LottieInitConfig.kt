...
object LottieInitConfig {
    ...
    fun initLottie(
        ...
        networkCacheFileName: String,
        enableSystraceMarkers: Boolean,
        enableNetworkCache: Boolean
    ) {
        runCatching {
            LottieConfig.Builder().run {
                setNetworkCacheDir(FileUtils.createCacheFile(application, networkCacheFileName))
                setEnableSystraceMarkers(enableSystraceMarkers)
                setEnableNetworkCache(enableNetworkCache)
                build()
            }
        }.onSuccess {
            Lottie.initialize(it)
        }.onFailure {
            Timber.e(it)
        }
    }
